...
// 
// Add 0x10000000 to the send index in order to access the ordering of the sends/hardware outputs in the mixer, which may be sparsely populated.
// 
...
// I_MIDIHWOUT_SLOT : int * : hint for slot index for MIDI HW output send, if enabled
...
// D_MUTE : double * : nonzero if the automation item is muted
...
// DIRTY : set to 1 if project was modified since last save, 0 if unmodified. Can also be set to 256 if project was modified and unsaved, but unmodified since it was last saved to a temporary file (if the appropriate options were configured in prefs).
...
// category is <0 for receives, 0=sends, 0x10000000 for sends (UI ordering), >0 for hardware outputs
//  sendidx is 0..n (to enumerate, iterate over sendidx until it returns NULL, except in the case of UI ordering where GetTrackNumSends can be used)
// parameter names:
...
// I_SENDMODE : int * : 0=post-fader, 1=pre-fx, 2=post-fx (deprecated), 3=post-fx, 8=pre-receive (not supported for hardware outputs)
...
// I_SLOT_HINT : int * : hint for slot index in UI, or -1 for unhinted.
...
REAPERAPI_DEF //==============================================
// GetTrackNumSends
// returns number of sends/receives/hardware outputs - category is <0 for receives, 0=sends, 0x10000000 for UI-ordered sends/hardware-output list size (may be sparsely populated; note you can also query 0x10000001 which will return 0x10000000 if UI-ordered lists are supported), otherwise >0 for hardware outputs
...
REAPERAPI_DEF //==============================================
// GetTrackSendName
// send_idx>=0 for hw ouputs, >=nb_of_hw_outputs for sends. 
// Add 0x10000000 to the send index in order to access the ordering of the sends/hardware outputs in the mixer, which may be sparsely populated.
...
REAPERAPI_DEF //==============================================
// RemoveTrackSend
// Remove a send/receive/hardware output, return true on success. category is <0 for receives, 0=sends, 0x10000000 for UI-ordering of sends, or >0 for hardware outputs.
// See CreateTrackSend, GetSetTrackSendInfo, GetTrackSendInfo_Value, SetTrackSendInfo_Value, GetTrackNumSends.
...
REAPERAPI_DEF //==============================================
// SetTrackSendUIPan
// send_idx<0 for receives, >=0 for hw ouputs, >=nb_of_hw_outputs for sends.
// Add 0x10000000 to the send index in order to access the ordering of the sends/hardware outputs in the mixer, which may be sparsely populated.
// isend=1 for end of edit, -1 for an instant edit (such as reset), 0 for normal tweak.
...
REAPERAPI_DEF //==============================================
// TrackFX_CopyToTrack
// Copies (or moves) FX from src_track to dest_track. Can be used with src_track=dest_track to reorder, FX indices have 0x1000000 set to reference input FX. dest_fx can also have 0x800000 set to specify an unused FX slot. FX indices for tracks can have 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to them, in which case they will be used to address FX in containers. To address a container, the 1-based subitem is multiplied by one plus the count of the FX chain and added to the 1-based container item index. e.g. to address the third item in the container at the second position of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm with parent_container and container_item.X.
...
// chain_index_to_slot : calculates the effective slot.
// chain_slot_to_index : calculates the index for FX in slot provided by the parameter. If no FX is present at this slot, will return empty:x where x is the index that would be used to fill this slot.
// 
// 
...
// slot_hint : index of preferred slot in mixer view, or -1 if not set.
//  FX indices for tracks can have 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to them, in which case they will be used to address FX in containers. To address a container, the 1-based subitem is multiplied by one plus the count of the FX chain and added to the 1-based container item index. e.g. to address the third item in the container at the second position of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm with parent_container and container_item.X.

...
const int* OscMessageRead::PopIntArg(bool peek, bool peekIfLast)
{
  if (!m_msgok) return 0;
  if (m_type_ptr >= m_type_end) return 0;
  ...
  int* p=(int*)m_arg_ptr;
  if (!peek && (!peekIfLast || m_arg_ptr + sizeof(int) < m_arg_end))
  {
    ++m_type_ptr;
...
const float* OscMessageRead::PopFloatArg(bool peek, bool peekIfLast)
{
  if (!m_msgok) return 0;
  if (m_type_ptr >= m_type_end) return 0;
  ...
  if (!peek && (!peekIfLast || m_arg_ptr + sizeof(float) < m_arg_end))
  {
    ++m_type_ptr;
...
const char* OscMessageRead::PopStringArg(bool peek, bool peekIfLast)
{
  if (!m_msgok) return 0;
  if (m_type_ptr >= m_type_end) return 0;
  ...
  if (!peek)
  {
    ...
    if (!peekIfLast || m_arg_ptr + n < m_arg_end)
    {
      ++m_type_ptr;
      m_arg_ptr += n;
    }
...
void OscMessageRead::DebugDump(const char* label, char* dump, int dumplen) 
{
  ...
  if (!label) label="";
  lstrcpyn_safe(dump, label, dumplen);
  lstrcatn(dump, m_msg_ptr, dumplen);
  ...
  if (*t == ',')
  {    
    snprintf_append(dump, dumplen, " [%s]", t+1);
    ...
    {  
      if (*t == 'i')
      {
        snprintf_append(dump, dumplen, " %d", *(int*)a);
        a += sizeof(int);
      }
      else if (*t == 'f')
      {
        snprintf_append(dump, dumplen, " %f", *(float*)a);
        a += sizeof(float);
      }
      else if (*t == 's')
      {
        snprintf_append(dump, dumplen, " \"%s\"", a);
        a += pad4((int)strlen(a));        
      }
      else
      {
        snprintf_append(dump, dumplen, " %c:(unknown argument type)", *t);
        break;
      }
    }
  }
